    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if args.with_tracking and completed_steps % args.tb_scalar_log_interval == 0:
                accelerator.log({"train/loss": loss.item()}, step=completed_steps)

                # weights inf-norm and kurtosis
                for name, module in model.named_modules():
                ...
                for layer_idx in range(num_layers):
                    ...
                    if hasattr(self_attn_layer, 'last_gate_avg_prob') and self_attn_layer.last_gate_avg_prob is not None:
            ...
            #     # wandb와 tensorboard 트래커 찾기
            #     wandb_tracker = None
            #     tb_tracker = None
            #     for tracker in accelerator.trackers:
            #         if tracker.name == "wandb":
            #             wandb_tracker = tracker
            #         elif hasattr(tracker, 'writer'):  # tensorboard tracker
            #             tb_tracker = tracker.writer
